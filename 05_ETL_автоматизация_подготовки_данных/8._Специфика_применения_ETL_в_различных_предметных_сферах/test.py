...
def get_yandex_wether(**kwargs):
    ...
    headers = {
        'X-Yandex-API-Key': os.getenv('YANDEX_API_KEY')  # Загружаем ключ из переменной окружения
    }
    response = requests.get(url, headers=headers)

    if response.status_code == 200:
        temp = response.json()['fact']['temp']
        print(f"Yandex Weather: {temp}")
        ti.xcom_push(key='wether', value=temp)
    else:
        print(f"Ошибка API Yandex: {response.status_code}, {response.text}")
...
def get_open_wether(**kwargs):
    ...
    params = {
        'lat': 55.749013596652574,
        'lon': 37.61622153253021,
        'appid': os.getenv('OPENWEATHER_API_KEY')  # Загружаем ключ из переменной окружения
    }
    ...
    if response.status_code == 200:
        ...
        print(f"OpenWeather: {temp}")
        ti.xcom_push(key='open_wether', value=temp)
    else:
        print(f"Ошибка API OpenWeather: {response.status_code}, {response.text}")


def get_wether(**kwargs):
    ti = kwargs['ti']

    yandex_temp = ti.xcom_pull(task_ids='yandex_wether', key='wether')
    open_temp = ti.xcom_pull(task_ids='open_wether', key='open_wether')

    print(f"Yandex: {yandex_temp}°C, OpenWeather: {open_temp}°C")
